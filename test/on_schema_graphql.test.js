...
const client = ()=>{
  return new GraphQLClient("http://localhost:4000", { headers: {
    userID:get_userID()  //临时在headers中传入userID，服务器端取此身份
  } })
}
...
test('#创建一个用户u1', async () => {   
  const query = `mutation{
    createUser(name: "huangyong", email:"${_run.email}") {
      id,
      name,
      email
    }
  }`
  const {createUser} = await client().request(query)
  console.log(createUser)
  expect(createUser.email).toBe(_run.email)
  _run.userID = createUser.id
})
